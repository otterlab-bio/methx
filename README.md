...
CpG extraction is a one-time step per genome, and it reports exactly what it found:

```text
$ methx extract-cpgs --genome tiny.fa --output tiny_cpgs.ron
INFO methx: Extracting CpG sites from genome: tiny.fa
-Done. Extracted 12 CpGs from 1 contigs.
CpG data saved to: tiny_cpgs.ron
```

The RON file holds 0-based, end-exclusive coordinates — the internal representation the processor
uses, not the 1-based closed coordinates written to HDF5 later (excerpted):

```text
(
    cpgs: [
        (chr: "chr1", start: 1, end: 3, strand: '+'),
        (chr: "chr1", start: 3, end: 5, strand: '+'),
```

Keeping those two coordinate systems distinct is a documented contract of this tool rather than an
implementation detail, because the HDF5 side is read by `rhdf5` consumers.
...
```bash
git clone https://github.com/otterlab-bio/methx.git
cd methx
cargo build --release
...
MIT · [otterlab-bio/methx](https://github.com/otterlab-bio/methx)
